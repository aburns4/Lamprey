function [alpha_r]= CouplingFunction(numOscillators, forcingPosition,Aa,Ad,lambda_a,lambda_d,sigma)
    ...
    intra = [ 1  2  35.0;
              ...
              3  6  35.0];
    %%%%%
    ra      = Aa;            % Amplitude of ascending strength function.
    rd      = Ad;            % Amplitude of descending strength function.
    g_ratio = 20;            % Scaling factor for intersegmental strenghts.
    la      = lambda_a;      % Length constant of ascending strength function.
    ld      = lambda_d;      % Length constant of descending strength function.
...
%     end
    ...
    pars.n       = numOscillators;                  % Number of oscillators
    n = pars.n;
...
%     end
    ...
    leftIndex=find(w(:,1)==0);
    rightIndex=find(w(:,3)==0);
    other=w(:,2);
